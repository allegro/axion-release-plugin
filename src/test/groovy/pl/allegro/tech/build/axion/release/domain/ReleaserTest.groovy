...
class ReleaserTest extends RepositoryBasedTest {
    ...
    def "should release new version when not on tag"() {
        ...
        releaser.release(
            rules,
            new ReleaseBranchesConfiguration(false, 'main', ['main'] as Set),
            new ConfigurationCacheConfiguration(false, { _ -> } as Closure<String>)
        )
    ...
    def "should do release when releaseOnlyOnReleaseBranches option is set and current branch is in releaseBranchNames list"() {
        ...
        releaser.release(
            rules,
            new ReleaseBranchesConfiguration(true, 'main', ['main'] as Set),
            new ConfigurationCacheConfiguration(false, { _ -> } as Closure<String>)
        )
    ...
    def "should skip release when current branch is not in releaseBranchNames list"() {
        ...
        releaser.release(
            rules,
            new ReleaseBranchesConfiguration(true, 'feature/branch', ['main'] as Set),
            new ConfigurationCacheConfiguration(false, { _ -> } as Closure<String>)
        )
